...
### 17.3.5 实操：动手做一次贝叶斯更新

用思考题1的医学检测来追踪一次完整的更新过程。不是套公式，是看筹码怎么移动。

**设定**：
- 疾病患病率（先验）：$P(\text{患病}) = 0.01$，$P(\text{健康}) = 0.99$
- 检测灵敏度（似然）：$P(\text{阳性} \mid \text{患病}) = 0.90$
- 检测特异度：$P(\text{阴性} \mid \text{健康}) = 0.95$，所以 $P(\text{阳性} \mid \text{健康}) = 0.05$

**第一步**：先验筹码。想象有10000个人的群体。按先验，约100人患病，9900人健康。

**第二步**：检测阳性来了。在100个患病者中，约90人检测阳性。在9900个健康者中，约 $9900 \times 0.05 = 495$ 人检测阳性。

**第三步**：重新分配信念。检测阳性的人群总共 $90 + 495 = 585$ 人。其中真正患病的有90人。所以你更新后的患病信念是：
$$P(\text{患病} \mid \text{阳性}) = \frac{90}{585} \approx 0.154 = 15.4\%$$

**用公式验证**：
$$P(\text{患病} \mid \text{阳性}) = \frac{0.90 \times 0.01}{0.90 \times 0.01 + 0.05 \times 0.99} = \frac{0.009}{0.009 + 0.0495} \approx 0.154$$

两个算法一致。

**关键观察**：检测阳性了，但患病概率只有15.4%。大多数人直觉会估到80%以上。为什么差这么多？因为疾病太罕见了（1%）——即使检测灵敏度高，阳性结果中大部分仍然是健康人的假阳性，简单说就是因为健康人的基数太大（99%），假阳性（5%）产生的阳性人数（495）远超真阳性人数（90）。

**再做一次检测**：把15.4%作为新的先验，第二次检测也是阳性。

$$P(\text{患病} \mid \text{两次阳性}) = \frac{0.90 \times 0.154}{0.90 \times 0.154 + 0.05 \times 0.846} = \frac{0.1386}{0.1386 + 0.0423} \approx 0.766$$

两次阳性后，患病概率跳到了76.6%。先验从1%->15.4%->76.6%——两次证据，信念发生了剧烈变化。

**如果基础患病率是10%（高风险人群）呢？** 一次阳性后：
$$P(\text{患病} \mid \text{阳性}) = \frac{0.90 \times 0.10}{0.90 \times 0.10 + 0.05 \times 0.90} = \frac{0.09}{0.09 + 0.045} = 0.667$$

高风险人群（10%）一次阳性后患病概率66.7%，高于低风险人群（1%）两次阳性后的概率76.6%？不，76.6% > 66.7%。两次证据的累积，在低风险人群中给出了比高风险人群单次检测更高的确信。这说明证据的累积力量——足够多的证据可以压倒先验。

但这个例子也揭示了贝叶斯更新的一个默认前提：**你观察到了确定的证据$E$**。你知道检测结果是”阳性”，没有歧义，概率为1。但现实中，证据本身往往也不确定。这就是下一小节的话题。
...
### 17.4.5 Jeffrey 条件化：当证据本身也不确定

贝叶斯定理假设你观察到了一个**确定的事件** $E$——"检测结果是阳性"——概率1。但你看到的不总是确定的。你看到一个暗影掠过窗外——可能是鸟，可能是飞机，可能是幻觉。你的观察不是"我看到了一只鸟（概率1）"，而是"我对'有只鸟飞过'的信念从0.1升到了0.7"。

这种不确定的证据，如何处理？

**Jeffrey 条件化**（Richard Jeffrey, 1965）推广了贝叶斯更新：证据不是确定的事件 $E$ （概率跳到1），而是对证据分区 $\{E_1, E_2, \ldots, E_k\}$ 的信念发生了变化——从旧的 $P_{\text{old}}(E_i)$ 变为新的 $Q_{\text{new}}(E_i)$。那么对任意假设 $H$：

$$P_{\text{new}}(H) = \sum_{i=1}^k P_{\text{old}}(H \mid E_i) \cdot Q_{\text{new}}(E_i)$$

这个公式说的是：当你的信念在证据分区 $\{E_i\}$ 上发生了变化，你对 $H$ 的信念也随之变化——变化量是 $H$ 和每个 $E_i$ 的旧条件概率，乘以每个 $E_i$ 的新信念度。

**例子**：你听到窗外有动静（$S$）。你的信念：可能是鸟（$B$，概率上升到0.6），可能是风（$W$，概率上升到0.3），可能是其他（$O$，概率0.1）。你关心"窗外有活物"（$L$）这个命题。已知旧的条件概率：$P(L \mid B)=1$，$P(L \mid W)=0$，$P(L \mid O)=0.5$。则：

$$P_{\text{new}}(L) = 1 \times 0.6 + 0 \times 0.3 + 0.5 \times 0.1 = 0.65$$

Jeffrey条件化是贝叶斯更新在"证据不确定"时的推广。贝叶斯更新是它的极限情况——当某个$E_i$的新信念跳到1而其他的跳到0。

**为什么这很重要？** 因为现实中几乎所有的证据都是不确定的。你看到的、听到的、读到的信息，很少能以概率1被接受。Jeffrey条件化让概率逻辑能够处理"我看到了不确定的东西"——这是日常推理的常态，而不是例外。
