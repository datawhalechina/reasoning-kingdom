...
## 15.0 镜子命题游戏：当系统开始看见自己

第14章的公理印章游戏还算安全：你盖章，按规则移动，最后得到定理。现在我们给这个游戏加一面镜子。

规则改变如下：每一个公式、每一个证明、每一串符号，都可以被编码成一个自然数。于是系统里不只有“$1+1=2$”这种谈论数字的命题，还可以出现谈论“某个公式编号是否可证”的命题。

这就像一座图书馆突然拥有了自己的目录，而目录里的每一本书又能引用目录本身。游戏从这里开始变危险。

玩家要做三步：

1. **编号**：把公式变成数字，让语句成为系统内部可操作的对象。
2. **引用**：构造一个谓词 $\mathsf{Prov}(n)$，表示“编号为 $n$ 的公式可证”。
3. **照镜子**：制造一个命题 $G$，它说的不是外部世界，而是它自己：$G$ 不可证。

如果系统证明了 $G$，它就证明了一个声称“我不可证”的命题，于是系统说谎；如果系统不能证明 $G$，那么 $G$ 说的恰好是真的，但系统抓不到它。

:::details 这个游戏的形式化骨架
- **状态空间**：公式、证明序列，以及它们的哥德尔编号。
- **合法动作**：给符号串编码；在系统内部表达“某编号是证明”；构造自指命题。
- **转移规则**：从普通算术命题移动到关于“可证性”的算术命题。
- **胜利条件**：Hilbert 式梦想——系统既一致，又能捕捉所有算术真理。
- **失败模式**：出现 $G \leftrightarrow \neg\mathsf{Prov}(\ulcorner G \urcorner)$，系统被自己的镜像卡住。
:::

这个游戏切中的不是“有些题太难”。难题仍然可能有答案，只是你暂时没找到。哥德尔的危险更深：当系统强到足以描述自己的证明行为时，它会产生某些真命题，而这些真命题无法在系统内部被证明。

形式系统不是因为弱才失败，而是因为强到能照见自己，才裂开。

::: info 兔狲教授评
镜子命题游戏的残酷之处在于：镜子不是外敌。它是系统表达能力的一部分。你想让系统足够强，强到能谈论算术；它就顺手学会谈论自己。然后，裂缝出现。不是有人攻击了城堡，是城堡长出了自己的倒影。
:::

### 15.0.5 小型哥德尔编号：编码如何让系统长出镜子

哥德尔编号不是抽象的"把符号映射到数字"，而是可以手动演示的机械编码。下面给一个极小的系统做一遍。

考虑符号集：$P$（命题变元）、$\to$（蕴含）、$\neg$（否定）。分别编码为：
$$P \mapsto 1,\quad \to \ \mapsto 2,\quad \neg \mapsto 3$$

一个公式（符号串）被编码为对应质数幂的乘积。公式 $\neg P \to \neg\neg P$ 的符号序列是 $(\neg, P, \to, \neg, \neg, P)$：
$$\text{编号} = 2^3 \cdot 3^1 \cdot 5^2 \cdot 7^3 \cdot 11^3 \cdot 13^1$$

**关键性质**：质因数分解唯一——给定这个编号，你可以100%确定地恢复出原来的符号串。每个公式对应唯一编号，每个编号最多对应一个公式。

证明序列（公式的列表）再编码一层：$(F_1, F_2, \ldots, F_k)$ 映射到 $2^{\#F_1} \cdot 3^{\#F_2} \cdot \cdots \cdot p_k^{\#F_k}$。

现在系统内部可以表达 $\mathsf{Prov}(n)$——"存在一个证明序列，其最后一个公式的编号是 $n$"。但 $\mathsf{Prov}(n)$ 并不是什么神秘的新谓词——它就是一个关于自然数的算术命题，说的是"存在某个自然数满足某某质因数分解条件"。它完全在系统语言内——谈论乘法、加法、质数——只是通过编码，这些算术关系恰好对应了"某个公式可证"这个元数学概念。

**这面镜子的力量**：系统原本只会谈论 $1+1=2$ 这类数字命题。有了哥德尔编号，系统"顺手"获得了谈论自己的能力——因为"证明"被编码成了数字之间的关系。不是系统被设计了这面镜子，而是数字大到能编码系统自身时，镜子的出现是必然的。

::: info 兔狲教授评
哥德尔编号不是"编码技巧"——它展示了"自指"如何作为表达能力的必然副产品出现。任何能够谈论加法和乘法的系统，自动地能够谈论关于自身的某些事实。不是因为"设计"了自指，而是因为系统的表达能力超过了某个阈值，自指就成了不可避免的数学事实。
:::
...
回到镜子命题游戏，这一步相当于把“公式”做成了可以被系统拿在手里把玩的棋子。第14章的公理印章游戏只允许你在纸上写公式；现在，纸上的公式有了编号，编号又能回到纸上，被另一个公式谈论。游戏的状态空间突然多了一层：它不再只是“命题是什么”，而是“命题如何谈论命题”。

这就是危险的来源。镜子一旦装上，系统就不只是向外看世界，也开始向内看自己的证明行为。哥德尔句不是从天而降的怪物，它是镜子规则自然允许的一步棋。
