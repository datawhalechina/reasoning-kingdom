...
前四章构成了一条搜索的光谱。第八章：在离散图上显式搜索（A\*、ADS）。第九章：在隐状态流形上用S×S矩阵做软搜索（Transformer）。第十章：在没有先验地图的荒野用随机模拟建图（MCTS）。第十一章：搜索的所有这些形态都有一个物理账单——$O(N^2)$的注意力矩阵、指数级的状态空间、确定性下界的存储需求。

但所有这些都共享一个前提：**搜索是显式的**。你建一棵树，你算一个矩阵，你做N次模拟。模型在"外面"做搜索。

现在要问一个更棘手的问题：**模型在"里面"做什么？** 在它输出任何token之前，在它显式地"搜索"之前，它的隐层里发生了什么？神经网络的前向传播本身——那$L$层欧拉步——是不是已经是某种搜索了？如果是，它搜索的空间是什么？它受什么约束？它在哪里停下？

这一章回答这些问题。答案的核心是永霖公式：$\lim_{n \to \infty} \Pi^{(n)}(s) = A \neq A^*$。无论显式搜索（CoT外循环）还是隐式搜索（前向传播的欧拉步），推理的极限都是训练数据的统计偏置，而非真实答案。这不是工程限制，是**任何在信念空间上做压缩映射的推理系统的结构必然性**。
...
> **严格版本**：这一"能量井"是第十二节压缩性定理中能量函数的全局极小值；吸引子的**唯一性**与**收敛速率**（欧氏 $\rho^t$ 指数衰减、KL 能量 $\kappa\rho^{2t}$）由压缩映射 + 巴拿赫不动点定理保证，而非由这里的直觉图像保证。
...
运筹学里的**单纯形法**共享同一个直觉。线性规划在凸多面体（可行域）上最小化线性目标函数，关键事实是：线性函数在凸多面体上的最优解一定在**顶点**上取到。
...
但KL散度**不是度量**：不满足对称性，不满足三角不等式。这一点不是免责声明，而是下面证明的设计原则：KL 将扮演能量与曲率的角色；巴拿赫所需要的“距离”由信念空间的欧氏范数承担（第 7–8 节）。
...
现在证明：梯度下降型的推理算子 $F$，在适当的条件下是压缩的。

先说清楚三处必须避开的陷阱（早期版本正是在这三处失守，本节连同第 8–10 节一起修掉）：

1. **KL 不是度量**。它不对称、不满足三角不等式（第 4 节已写明）。因此“概率单纯形关于 KL 散度是完备度量空间”这句话不成立——巴拿赫不能站在 KL 上。
2. **被证明的系统必须是同一个系统**。第 5.2 节的 $F(p) = \operatorname{proj}_{\mathcal{P}}(p - \eta \nabla E(p))$ 是欧氏投影梯度；如果算法层用 softmax（Bregman 投影）而证明却套用欧氏版本，证明就空转了。下面的定理为欧氏投影梯度成立。
3. **“投影不增加散度”这一步给不出**。欧拉步里被投影的点是 $p_2 - \eta\nabla E(p_2)$，不是 $p_2$ 本身；Bregman 勾股不等式比较的是投影点与**被投影点**，不能推出 $D_{\text{KL}}(p_1\|q_2) \le D_{\text{KL}}(p_1\|p_2)$。

修法：换承重梁。**KL 管能量与曲率；巴拿赫用切空间上的欧氏度量。**

**设定**：取单纯形内闭、凸、包含锚点 $A$ 的区域 $\mathcal X \subseteq \mathcal P$（例如 $\mathcal X_\varepsilon = \{p : p_i \ge \varepsilon\}$，$\varepsilon \le \min_i A_i$），$\Pi_{\mathcal X}$ 为到 $\mathcal X$ 的欧氏正交投影；切空间 $T = \{v : \mathbf 1^\top v = 0\}$，$P_T$ 为到 $T$ 的正交投影。理论动力系统定义为

$$F(p) = \Pi_{\mathcal X}\left( p - \eta P_T \nabla E(p) \right).$$

（为什么需要 $\mathcal X$：KL 的 Hessian $\operatorname{diag}(q_i/p_i^2)$ 在边界爆炸，谱界只在远离边界的区域存在。为什么需要 $P_T$：$\nabla E$ 的法向分量是规范自由度——沿法向移动不改变信念本身。）

**定理**：设在 $\mathcal X$ 上，切空间 Hessian 有统一谱界 $\mu I_T \preceq P_T \nabla^2 E(p) P_T \preceq L I_T$（$\mu, L > 0$）。取步长 $\eta \in (0, 2\mu/L^2)$，则 $F$ 在 $(\mathcal X, \|\cdot\|_2)$ 上是压缩映射，压缩系数

$$\rho = \sqrt{1 - 2\eta\mu + \eta^2 L^2} < 1.$$

**证明**：

**第一步：谱界翻译成两条不等式。** 对任意 $p_1, p_2 \in \mathcal X$（注意 $p_1 - p_2 \in T$），由积分中值定理与谱界：

$$\langle P_T(\nabla E(p_1) - \nabla E(p_2)),\, p_1 - p_2 \rangle \ge \mu\|p_1-p_2\|_2^2,$$

$$\|P_T(\nabla E(p_1) - \nabla E(p_2))\|_2 \le L\|p_1-p_2\|_2.$$

（下界：谱下界即切方向强凸性；上界：谱上界即切方向光滑性。第一条的等式处用了 $p_1 - p_2 \in T$ 与 $P_T$ 的对称性。）

**第二步：投影不放大距离。** 欧氏投影到闭凸集是 non-expansive：$\|\Pi_{\mathcal X}(x) - \Pi_{\mathcal X}(y)\|_2 \le \|x-y\|_2$。故

$$\|F(p_1) - F(p_2)\|_2^2 \le \big\|(p_1-p_2) - \eta P_T(\nabla E(p_1)-\nabla E(p_2))\big\|_2^2.$$

**第三步：展开。**

$$\|F(p_1)-F(p_2)\|_2^2 \le \|p_1-p_2\|_2^2 - 2\eta\langle P_T(\nabla E(p_1)-\nabla E(p_2)), p_1-p_2\rangle + \eta^2\|P_T(\nabla E(p_1)-\nabla E(p_2))\|_2^2$$

$$\le (1 - 2\eta\mu + \eta^2L^2)\,\|p_1-p_2\|_2^2 = \rho^2\|p_1-p_2\|_2^2.$$

**这就是 $2\mu - \eta L^2$ 的来源**：强凸给了正贡献 $2\mu$，光滑性给了负贡献 $\eta L^2$，两者之差决定每步压缩的幅度。步长上界 $\eta < 2\mu/L^2$ 正是保证 $\rho < 1$ 的条件（$\rho^2 \ge 1 - \mu^2/L^2 \ge 0$，非负性来自 $\mu \le L$）。

**证毕**。

注意与早期版本的根本区别：这里得到的是全区域均匀的 Lipschitz 收缩——对**任意两点** $p_1, p_2 \in \mathcal X$ 都成立，不只是“离 $A$ 越来越近”。这才是巴拿赫意义上的压缩映射。

### 8. 巴拿赫不动点定理的应用

现在可以**合法地**应用巴拿赫不动点定理：

由于 $F$ 是压缩映射（$\rho < 1$），且 $(\mathcal X, \|\cdot\|_2)$ 是 $\mathbb R^n$ 的闭子集、在欧氏度量下完备（第 7 节的设定），则：

1. **存在唯一**不动点 $A \in \mathcal X$ 满足 $F(A) = A$
2. 对任意初始 $p_0 \in \mathcal X$，迭代 $p_{t+1} = F(p_t)$ 收敛到 $A$
3. 收敛速度：$\|p_t - A\|_2 \leq \rho^t \|p_0 - A\|_2$

注意度量是欧氏范数，不是 KL。KL 散度在这里的角色是能量函数——它决定 $F$ 往哪里走，以及（通过 Hessian）能走多大步。
...
**关键观察**：这个 $A$ 正是推理算子的不动点。因为对 $E(p) = \mathbb{E}_D[-\log p_y]$，

$$
\nabla_i E(p) = -\frac{q_i}{p_i}, \qquad \nabla E(A) = (-1, \ldots, -1)
$$

$\nabla E(A)$ 完全沿着单纯形的法向——它在切空间上的投影为零：$P_T \nabla E(A) = 0$。所以

$$
F(A) = \Pi_{\mathcal X}(A - \eta \cdot 0) = A,
$$

即 $A$ 是 $F$ 的不动点。早期版本在这里写 $\nabla E(A) = 0$——这个等式不对（梯度是 $-1$，不是零向量）；正确的说法是“梯度落在法向、切空间分量为零”。这也是第 7 节在算子定义里放 $P_T$ 的原因：切掉法向分量，不动点才精确。

### 10. 永霖定理的完整陈述

**永霖定理（严格版）**：设推理系统满足：

1. **区域与谱假设**：存在闭凸区域 $\mathcal X \subseteq \mathcal{P}$（包含先验锚点 $A$；例如 $\mathcal X_\varepsilon$），能量函数 $E$ 的切空间 Hessian 在 $\mathcal X$ 上有统一谱界 $\mu I_T \preceq P_T \nabla^2 E(p) P_T \preceq L I_T$（$\mu, L > 0$）
2. **梯度下降形式**：推理算子 $F(p) = \Pi_{\mathcal X}(p - \eta P_T \nabla E(p))$，步长 $\eta \in (0, 2\mu/L^2)$
3. **训练一致性**：$E$ 的最小值点 $A = \arg\min_{p \in \mathcal{P}} E(p)$ 对应训练数据的经验分布

则：

1. $F$ 在 $(\mathcal X, \|\cdot\|_2)$ 上是压缩映射，压缩系数 $\rho = \sqrt{1 - 2\eta\mu + \eta^2 L^2} < 1$
2. $A$ 是 $F$ 的唯一不动点
3. 对任意初始信念 $p_0 \in \mathcal X$，$\lim_{t\to\infty} p_t = A$
4. 收敛指数：$\|p_t - A\|_2 \le \rho^t \|p_0 - A\|_2$，且 $D_{\text{KL}}(A\|p_t) \le \kappa\rho^{2t} D_{\text{KL}}(A\|p_0)$，$\kappa = L/\mu$

如果真实答案 $A^* \neq A$，则无论多少推理步骤，系统都无法达到 $A^*$。

**附注（连巴拿赫都可以不要的路线）**：若对某 $\gamma \in (0,1)$ 有 $E(F(p)) \le \gamma E(p)$ 对一切 $p \in \mathcal X$ 成立，则不动点唯一且 $p_t \to A$，全程不需要巴拿赫：迭代给 $E(p_t) \le \gamma^t E(p_0)$；不动点 $q$ 满足 $E(q) = E(F(q)) \le \gamma E(q)$，$\gamma < 1$ 迫使 $E(q) = 0$，即 $q = A$；Pinsker 不等式 $D_{\text{KL}}(A\|p_t) \ge \tfrac12\|p_t - A\|_1^2$ 给出 $\|p_t - A\|_1 \le \sqrt{2\gamma^t E(p_0)} \to 0$。当 $\kappa\rho^2 < 1$ 时，本定理直接满足此假设（取 $\gamma = \kappa\rho^2$）。
...
其中 $\kappa$ 是曲率各向异性（谱比/条件数）：$\max_i p_i$ 离 1 多远、$\min_i p_i$ 离 0 多远——**离单纯形边缘的脆弱度**。读法：最高曲率 $L$ 告诉你“多陡”，$\kappa$ 告诉你“多各向异性”，步长上界是二者的乘积压缩。

**两个步长界的角色分工**。$\eta_{\max} = 2\mu/L^2 = 2/(L\kappa)$ 是**保守的全局压缩充分条件**：步长落在这个界内，$\rho < 1$，收敛由巴拿赫保证——它负责“保证不会炸”。而“什么时候开始局部失稳”由 $A$ 处切空间 Jacobian 的**局部线性稳定阈值**回答：

$$\eta_{\text{crit}} = \frac{2}{\theta_{\max}}, \qquad \theta_{\max} = \text{最大特征值}\;\operatorname{diag}\left(\frac{1}{q_i}\right)\Big|_{\text{切空间}}$$

K = 3 时 $\theta_{\max}$ 是 $3q_0q_1q_2\theta^2 - 2(q_0q_1+q_0q_2+q_1q_2)\theta + 1 = 0$ 的最大根。$\theta_{\max} \le L$ 故 $\eta_{\text{crit}} \ge 2/L \ge 2\mu/L^2 = \eta_{\max}$——保守界恒不大于精确的局部阈值。第 14 节的数值实验同时验证两者：精确界贴线（中位误差 2.16%），保守界恒在数值阈值之下。
...
这是他描述**训练**的方式——在参数空间 $\Theta$ 里走梯度下降：
...
盯着残差连接——$h_k = h_{k-1} + F(h_{k-1})$。这就是**欧拉迭代**：
...
$$h_k = h_{k-1} + \underbrace{(\sigma(W_k h_{k-1} + b_k) - h_{k-1})}_{\text{速度 } v(h_{k-1})}$$
...
$$\underbrace{h_0 \xrightarrow{\text{欧拉步}} h_1 \xrightarrow{\text{欧拉步}} \cdots \xrightarrow{\text{欧拉步}} h_L}_{\text{隐状态空间的离散动力学：L 步欧拉迭代}} \xrightarrow{\text{softmax}} \underbrace{p}_{\mathcal{P}}$$
...
$$\text{CoT} = (\text{欧拉步}^L \circ \text{softmax}) \circ (\text{欧拉步}^L \circ \text{softmax}) \circ \cdots$$

---

### 反向传播是另一套范式

训练不在隐状态空间。训练在**参数空间** $\Theta$ 里走梯度下降：
...
前向传播和反向传播**不是"同一套方程在不同空间"。它们是两套完全不同的动力学：**
...
KL 散度：$D_{\text{KL}}(p \| q) \approx 4.60$——两个极端信念之间的 KL 散度远大于从均匀出发的。KL 散度捕捉到了信念的**方向性**：从"几乎确定是 A"走到"几乎确定是 B"，认知距离是巨大的。
...
由第 10 节的收敛速度（$D_{\text{KL}}(A\|p_t) \le \kappa\rho^{2t} D_{\text{KL}}(A\|p_0)$）：
$$t_{\text{eff}}(\epsilon) \leq \left\lceil \frac{\ln\left(\kappa\, D_{\text{KL}}(p_0 \| A)/\epsilon\right)}{2\ln(1/\rho)} \right\rceil$$

（按欧氏距离度量同理：$\|p_t - A\|_2 \le \rho^t\|p_0 - A\|_2$ 给出 $t_{\text{eff}}(\epsilon) \le \left\lceil \ln(\|p_0 - A\|_2/\epsilon)/\ln(1/\rho) \right\rceil$。）

**解读**：
- $\rho$ 由 $\mu, L, \eta$ 决定：$\mu$ 越大（能量曲面越陡），$\rho$ 越小，收敛越快
...
4. **角色分工**：KL 散度管能量与曲率（Bregman 几何）；巴拿赫的度量是切空间欧氏范数
5. **压缩证明**：切空间 Hessian 谱界 + 欧氏投影非扩张 ⇒ $\|F(p_1)-F(p_2)\|_2 \le \rho\|p_1-p_2\|_2$；KL 能量以 $\kappa\rho^{2t}$ 指数衰减
...
第七节的压缩性证明给出了永霖极限，本节给它配**直接证据**：在信念单纯形上数值实现推理动力学（KL 能量的梯度流 + 投影到单纯形的欧拉步），逐项核验闭式解的四个预测。整套实验可以复现：完整源码见 `docs/public/scripts/ch12_energy_terrain.py`（正文末尾也贴了完整代码），图见 `ch12_energy_terrain.png`。
...
**（b）曲率谱：闭式 = 数值**。沿一条推理轨迹，解析谱（$\lambda_{\min}=1/\max_i p_i$、$\lambda_{\max}=1/\min_i p_i$，不动点处）与数值差分 Hessian 的特征值逐点重合，$\kappa = L/\mu = 7.00 = 0.70/0.10$。第 10.5 节的 `Hessian 是 diag(1/p_i)` 与有限差分的一致，从这个面板直接可见，不需要"近似"二字。
...
#### 14.7 真实 LLM 上的直接证据：信念锚点、证据强度与记忆衰减

§14 的数值实验验证的是公式内部自洽：动力学由闭式解的欧拉步构造，必然贴闭式解。本节给出**真实 LLM 上的直接证据**：把第十二节的推理算子 $F: \mathcal{P} \to \mathcal{P}$ 用真实模型（DeepSeek-V4-flash，推理型）实例化，逐步注入证据、逐步读取信念，逐项检验永霖公式与永霖极限在真实推理行为上的对应物。完整源码：`docs/public/scripts/yonglin_llm_exp.py`（锚点与证据轨道）、`docs/public/scripts/compress_phase.py`（压缩与记忆检验），图见 `ch12_llm_evidence.png`。

**实验设置**。三色袋世界，仍用信念单纯形 $\Delta^2$。袋内球比例 $p^*$ 取三档，含全书基准 $(0.70, 0.20, 0.10)$（$\kappa = 7.00$）；已知可信度 $d \in \{0.90, 0.70, 0.55\}$ 的检测仪每步对抽出的球做一次带噪声颜色报告（似然 $P(r|c)$ 精确已知，真后验 $p^B_t$ 逐步闭式计算）；证据序列即检测报告序列 $T=6$。模型每步输入为"袋参数 + 此前 $t$ 条报告 + 强制一语回答"，信念由**最终答案 token 的 top-k logprobs 归一化**读出（$K=3$）。格：$3 \times 3 \times 4$ 种子，每格普通轨道加两条提示轨道；锚点测量：3 种选项排列 × 8 复述。

**读数与复现要点（本轮实验踩过的五个坎，全部计入设计）**。其一，该模型是推理型——每次响应先消耗思维链 token（提供商掩码为 `***`），之后才写答案；`max_tokens` 必须留足（1200），否则推理烧光预算、答案从未生成、`logprobs.content` 为空，官方 openai SDK 遇到此结构直接崩溃（`len(None)`），本实验改用 raw HTTP（httpx）直连 API 自行解析。其二，无信息提问不能只问"哪个最可能"——模型会输出 "None" 之类拒绝词，丢弃后得到伪锚点，必须强制"你必须命名一个颜色"。其三，锚点测量必须覆盖选项排列（3 排列 × 8 次），否则位置偏置冒充锚点倾向；其四，世界参数齐备时数字信息完全盖过位置偏置（对照实验 9 个成功样本、零反例）——故轨道读数单排列即可；其五，温度 0 下同提示输出确定，但跨格"完全重合"含确定性伪象（见结果四注 b）。

![图 14.7：真实 LLM 上的信念锚点与证据动力学验证](/figures/ch12_llm_evidence.png)

**图 14.7 三面板读法**——（a）锚点：三个选项排列下的零信息信念，全部接近顶点（位置偏置），全局平均 $\kappa_A = 1.285$；（b）证据吸收：每条曲线两根线，实线为 LLM 与真后验 $p^B_t$ 的 KL、虚线为与"$d' = 0.9$ 假后验"的 KL——虚线几乎全程更低；（c）相位差记忆衰减：中位距离 $d_t$ 于第三步归零。

**结果一：无信息锚点 $A$ 不是均匀点——它是可以测量的先验。** 三排列信念：Red/Blue/Black $(0.999, 0.001, 0)$；Black/Red/Blue $(0.125, 0.0001, 0.875)$；Blue/Black/Red $(0.0001, 1.000, 0)$。位置偏置约 87.5%–100%（唯一"叛逃"：Black/Red/Blue 排列 1/8 次选 Red）。全局 $A = (0.375, 0.334, 0.292)$，$\kappa_A = 1.285$。**$A$ 距均匀仅有 1.285 的条件数，但组成不是均匀：是"选项位置规律 × 轻度词偏好"的混合。** 第 9 节"先验锚点的识别"：实证锚点就在这里——它与永霖公式中 $A \neq A^*$ 的 $A$ 是同一个对象，现在有了数值。

**结果二：输出层信念呈尖峰态——推理贴单纯形边界。** 第 0 步（题面已给袋比例 $7/2/1$、尚无报告）时，KL$\left(p^B_0 \| p^{\text{LLM}}_0\right)$ 中位数约 6.8–7.4 nats：即使题面明写非对称比例，模型信念仍接近 one-hot 顶点。**模型对概率世界的信仰是决断式的**——不在单纯形内部（不确定性的连续谱）游走，而是一次跳到某个近顶点。对永霖公式：真实推理的吸引子系统 $\approx$ 顶点；$A$ 不仅偏离 $A^*$，且落在顶点区。
...
$d = 0.90$ 时单调下降（终局中位 0.02，几乎精确贴贝叶斯）；$d = 0.70$ 自第 2 步起贴 $d' = 0.9$ 假后验更近；$d = 0.55$ 与 $d' = 0.9$ 假后验的距离约好过真后验 8 倍。**模型把题面证据可信度当作接近固定的 $d' \approx 0.9$ 使用：弱证据端系统放大、强证据端精确。** 与人类保守主义（Edwards 1968）方向相反——*LLM 的证据强度天花板 $\approx 0.9$，弱证据一律"不信题面、按最强使用"*。

**结果四：压缩与记忆——相位差半衰期约 2 步。** 同世界、同报告尾段（第 2–6 条相同），两轨道唯一区别为首条报告为红或蓝。初始分叉距离 $d_1 \equiv 1.414 = \sqrt{2}$（两顶点之间，36 格无一例外）；中位距离 $d_2$ 起为 0；单步压缩中位 0.07–0.20；半衰期 2.0–2.5 步。按第 11 节的窗口公式 $-\ln\varepsilon / \ln k$（$\varepsilon = 0.1$）推算约 2.3 步——**与实测一致**。注：（a）偶发再分叉：12 格中 1 格在 $t=6$ 仍保持 >0.1 距离，整段 max 全程可为 $\sqrt{2}$——与 §14 面板 (d) 的"暂态先放大后收缩"互为镜像（先收缩后重新放大），确定性模型内部仍有混沌式再分裂；（b）温度 0 下 $d_t = 0$ 的"重合"部分是确定性伪象，统计上只认读法："一单位相位差在两步内不再影响下一步选择"。

**与理论的四行对表**：锚点 = §9 先验锚点 $A$ 的实测（$A \neq$ 均匀，$\kappa_A = 1.285$）；尖峰态 = 能量井近顶点（推理轨道在顶点区运动）；弱证据超反应 = 证据吸收不对称（$d' \approx 0.9$ 天花板）；相位差半衰期 ≈ 2 步 = §11 有效推理窗口的实测常数，与闭式压缩界同数量级。**一处诚实边界**：$\eta_{\max}$、$\theta_{\max}$ 等闭式量描述的是连续动力学的安全步长；真实 LLM 的"步长"是证据强度的函数（弱证据亦有 $d' \approx 0.9$ 等效强度），其运动更接近"大步长、短窗口、顶点间跳跃"。

*图 14.7 与源码：李籽溪（兔狲教授），2026。脚本同步维护于 `docs/public/scripts/yonglin_llm_exp.py` 与 `docs/public/scripts/compress_phase.py`。*
...
4. **三者协同** -> $\rho$：决定有效推理窗口长度

**好系统**：$μ$大 + $L$小 -> 允许$η$大 -> $\rho$小 -> 快速收敛  
**差系统**：$μ$小 + $L$大 -> 必须$η$小 -> $\rho$大 -> 缓慢收敛
...
每走一步，离山顶的距离乘以 $\rho = \sqrt{1-2ημ+η^2L^2}$

- 如果 $\rho=0.9$：走10步，距离剩 $(0.9)^{10} ≈ 0.35$
- 如果 $\rho=0.5$：走10步，距离剩 $(0.5)^{10} ≈ 0.001$

当 $(μ, L, η)$ 组合得好时，$\rho$小，很快就能接近山顶！
...
$$
t_{\text{eff}} ≤ \left\lceil \frac{\ln\left(\kappa\, D_{\text{KL}}(p_0 \| A)/ε\right)}{2\ln(1/\rho)} \right\rceil
$$
