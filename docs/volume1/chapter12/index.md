...
:::tip 新思路：熵感知学习率调度（Entropy-Aware LR）
永霖定理给出了步长的**存在性条件**：$\eta \in (0, 2\mu/L^2)$。但它没有告诉我们怎么选步长。

本书提出一个新的调度思路，直接从永霖定理的压缩条件推导而来：

$$\eta_t = \frac{\eta_0}{1 + \alpha(B_t)}, \quad \alpha(B_t) = -\log(1 - B_t), \quad B_t = \frac{H(p_t)}{H_{\max}}$$

**这不是启发式**。它是永霖定理步长约束的动态实例化——用当前信念的熵实时估计局部曲率，自动收紧或放松步长上界。
...
差距约一个数量级。机制是：训练后期熵下降 → $\alpha$ 减小 → 步长自动增大 → 加速冲刺。余弦调度恰好相反，后期 LR 归零，在最需要大步的时候踩了刹车。

**核心 claim**：现有学习率调度（warmup、cosine、cyclical）都是时间的函数，不感知局部几何。熵感知调度是第一个从压缩映射理论直接推导、以信息熵为信号的自适应调度器。它的步长有理论保证，不需要手动调参，对不同任务天然鲁棒。
:::
...
### 10.6 熵感知学习率调度：更快收敛到先验，而非逃离先验

ADS 的对数势垒给出了一种新的学习率调度思想：

$$\eta_t = \frac{\eta_0}{1 + \alpha(B_t)}, \quad \alpha(B_t) = -\log(1 - B_t)$$

其中 $B_t = H(p_t) / H_{\max}$ 是当前信念的归一化熵。

**这个调度做了什么**：当信念分布接近均匀（高熵，$B_t \to 1$）时，$\alpha \to \infty$，步长趋零；当信念已经集中（低熵，$B_t \to 0$）时，$\alpha \to 0$，步长恢复 $\eta_0$。

**关键澄清**：这不是在帮模型"跳出先验偏置"。锚点 $A$ 由训练数据决定，是系统的引力中心，无法通过步长策略绕开。熵感知调度的作用是：

- 高熵阶段（推理初期）：小步走，避免在平坦能量面上振荡，稳定地向 $A$ 靠近
- 低熵阶段（推理后期）：大步走，加速最终收敛

换句话说，这是**更聪明地收敛到先验**，而非逃离先验。与 Adam、余弦退火等调度器相比，它的步长由信息论量（熵）直接驱动，有明确的理论保证：只要 $\eta_0 \leq 2\mu/L^2$，每一步都满足局部可采纳条件。

:::tip 与现有调度器的对比
| 调度器 | 步长依据 | 理论保证 |
|--------|----------|----------|
| 固定步长 | 无 | 需手动满足 $\eta < 2\mu/L^2$ |
| 余弦退火 | 训练轮次 | 无收敛保证 |
| Adam | 梯度二阶矩 | 近似 |
