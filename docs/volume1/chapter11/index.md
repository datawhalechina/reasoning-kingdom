...
前两章画了两条搜索的弧线。第八章的A\*：在一个固定图上，用可采纳的启发函数换最优性保证——代价是必须探索所有"可能最优"的路径。第九章的Transformer：在隐状态流形上，用S×S矩阵做软搜索——代价是$O(N^2)$的计算复杂度。第十章把搜索推到了没有先验地图的荒野——MCTS用随机模拟建图，代价是每次决策需要数千次模拟。

这些代价——计算时间、内存占用、能量消耗——不是意外。它们是**搜索的账单**。每一种搜索策略都在用一个维度的资源换取另一个维度的能力。这一章把账单摊开来看：推理不仅是逻辑问题，也是经济问题。它消耗物理资源，因此受物理定律约束。
