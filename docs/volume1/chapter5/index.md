...
:::details 经验风险最小化（ERM）：训练模型在做什么？
**经验风险最小化**是大多数机器学习训练的底层逻辑，用大白话说就是：**在训练数据上，让模型的预测误差尽可能小**。

几个关键词：
- **假设 $h$**：模型本身（比如一个神经网络），给定输入 $x$，输出预测 $h(x)$
- **损失函数 $\ell$**：量化"预测有多差"的函数，比如预测"猫"但标签是"狗"，损失就高；预测正确，损失就低
- **经验风险 $\hat{R}(h)$**：在所有训练样本上的平均损失——最小化这个，就是"让模型在训练集上尽量答对"

**核心问题**：最小化训练误差 ≠ 真的理解了规律。模型可能只是"记住了答案"（过拟合），或者学到了表面模式而非底层机制。这正是本章要追问的：ERM 学到的是什么？
:::
