...
第八章讲了A\*——在一个固定的图上，用启发函数指引搜索方向。第九章讲了Transformer——在隐状态流形上，用S×S矩阵度量状态之间的相关性，每层走一步软搜索。这两种搜索有一个共同的盲区：**它们依赖一个已经铺好的"路网"**。A\*的图是人画的，Transformer的隐状态度量是训练数据喂出来的。

但如果你被扔进一片完全没有路的荒野呢？没有图，没有度量，没有可采纳的启发函数——你只知道起点、终点、以及"每走一步，看看周围，判断往哪走"的能力。

这就是蒙特卡洛树搜索（MCTS）的战场。它不依赖任何先验地图。它靠的是**随机模拟**——往各个方向扔石子，看哪条路走得通，然后回来告诉决策者。它不是在已知的地形上搜索，而是在**搜索中建图**。

如果把A\*比作拿着地图的登山者，Transformer比作在流形上滑行的粒子，那么MCTS就是一个蒙着眼睛在迷宫里探路的人——每一步都先伸手摸一摸，摸完了再决定往哪走。三个算法，三种搜索哲学：**A\*是先验几何，MCTS是经验采样，Transformer是隐式度量。** 这一章讲第二种。
...
class CausalPlanner:
    def __init__(self, scm):
        self.scm = scm  # NeuralSCM实例
    
    def plan(self, E_init, target, interv_nodes, lr=0.1, steps=80):
        """
        E_init: (N, D) 初始嵌入
        target: {var_name: desired_norm} 目标范数
        interv_nodes: [var_name] 可干预的变量
        """
        a = {node: 1.0 for node in interv_nodes}  # 初始干预强度
        ...
        for step in range(steps):
            ...
            for node in interv_nodes:
                ...
                a[node] = max(0.0, min(2.0, a[node]))  # 约束在[0,2]
...
MCTS 在井字棋上近乎完美，因为它穷举了可能性。但穷举是昂贵的——每一步需要数百次模拟。如果状态空间有连续结构，为什么要在离散树上爬？

让我们做一个思想实验：**把棋盘状态编码为嵌入向量，棋盘规则编码为因果图，搜索变成在嵌入空间中的梯度下降**。

这就是 CocDo 项目做的事情——一个因果世界模型，把 Pearl 的 do-calculus 和 COC 类型论融合在一起。

### 8.1 因果世界算子：do() 作为 β-归约

CocDo 的核心是一个**神经结构因果模型**：每个变量（井字棋的9个格子）是一个 D 维嵌入向量，因果边连接共享胜利线的格子。
...
def mcts_move(board, player, n_rollouts=500):
    # 构建搜索树，UCB1选择，随机模拟，回传统计
    # 时间复杂度：O(rollouts × depth)
    return best_move
```
...
**能量函数**：`E(a) = Σⱼ (‖E_next[j]‖ - target_j)²`。梯度下降找到使嵌入范数接近目标的干预值。

### 8.3 实验结果：先手全胜，后手部分失败

运行 `python examples/demo_tictactoe.py --games 50`：
...
**解读**：
- **先手时梯度规划全胜**：从空棋盘开始，梯度信号清晰——中心嵌入范数最大，对应最优第一步。
- **后手时部分失败**：当棋盘复杂（需要多步前瞻），梯度可能陷入局部最优，错过强制阻挡。

**这不是 bug，是特征**：梯度规划捕捉了**连续启发式**——"这个位置看起来多好"，而不是"搜索所有可能结局"。它用嵌入空间中的几何距离代替了树搜索中的胜率统计。
...
观察：
1. 梯度规划在简单局面（开局、直接胜利/阻挡）表现完美
2. 在复杂局面（需要2步以上前瞻）可能失误  
3. 计算速度对比：梯度规划 O(80×9×16) ≈ 11520 次浮点运算 vs MCTS O(200×深度) 次状态评估

**思考题**：
- 梯度信号在什么情况下会误导？（当最优路径需要暂时"变坏"）
- 因果图编码了哪些游戏知识？（胜利线的拓扑结构）
- 这暗示了 AI 规划的未来方向是什么？（连续表示+梯度优化 vs 离散搜索）

::: info 兔狲教授评
MCTS 是离散的穷举，CausalPlanner 是连续的启发。这不是谁更好的问题，而是**搜索的两种本体论**：一个在状态树上爬，一个在嵌入空间中流。梯度下降不是"近似的树搜索"，它是完全不同的东西——用向量空间中的距离代替了树中的胜率。承认这个区别，比假装它们在竞争更重要。
:::

## 十、大语言模型的搜索：从思维链到强化学习
...
### 10.2 DeepSeek-R1：干掉PRM，组内竞争强化学习
