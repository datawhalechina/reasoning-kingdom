...
:::details 知识获取瓶颈：为什么专家的知识很难写成规则？
"知识获取瓶颈"这个词说的是：**把人类专家头脑里的知识转化为计算机能处理的规则，这件事本身极其困难**。

原因有两层：
1. **专家说不清楚自己在做什么**：很多经验性判断已经自动化到了潜意识层面，专家自己都无法把它显式地描述出来（见下方"内隐知识"）。
2. **知识是情境依赖的**：同一条信息在不同语境下含义不同，但规则系统无法优雅地处理这种依赖关系。
...
:::details 内隐知识（Tacit Knowledge）：说不出来的那部分知识
...
这个假设在封闭、有界的领域（比如航班数据库）是合理的。但在开放的现实世界中，它非常危险：系统会把"我没见过这种情况"误认为"这种情况不存在"，然后自信地给出错误答案。
...
:::details 本体论、描述逻辑、语义网：这三个词是什么关系？
这三个词都是"知识如何在计算机里被结构化表示"的不同层面：

- **本体论（Ontology）**：在计算机科学里，它指的是"用形式化语言定义某个领域的概念体系和概念间关系"。比如医学本体论 SNOMED 定义了"疾病""症状""药物"之间的层级关系。
- **描述逻辑（Description Logic）**：构建本体论用的那套逻辑语言，比自然语言更精确，比完整的一阶逻辑更可计算。OWL（Web 本体语言）就基于描述逻辑。
...
在结束本章之前，让我们看看符号主义的遗产如何在现代AI中延续——以**知识图谱**的形式。

### 知识图谱：符号主义的规模化实现

知识图谱本质上是专家系统知识库的**规模化、结构化版本**：
- **实体**代替了专家系统中的"概念"
- **关系**代替了If-Then规则中的"条件"
- **三元组**（头实体-关系-尾实体）构成了基本的知识单元

但知识图谱面临与专家系统相似的挑战：

1. **构建成本**：需要大量人工标注或复杂的抽取算法
2. **覆盖度问题**：永远无法完整覆盖所有知识
3. **动态更新**：世界在变，知识图谱需要持续更新
4. **知识表示**：如何表示模糊、概率、上下文相关的知识？

### 知识图谱与大模型的对话

当前AI研究的一个前沿方向是**符号（知识图谱）与统计（大模型）的结合**。这不是简单的替代关系，而是**互补关系**：

| 维度 | 知识图谱 | 大模型 |
|------|----------|--------|
| **知识表示** | 显式、结构化、离散 | 隐式、分布式、连续 |
| **可解释性** | 高：清晰的三元组结构 | 低：黑盒的向量运算 |
| **推理方式** | 符号逻辑推理 | 统计模式匹配 |
| **知识获取** | 人工构建/抽取 | 从文本中隐式学习 |
| **动态性** | 更新成本高 | 可通过微调适应 |
...
1. **检索增强生成（RAG）**：用知识图谱作为外部记忆，为大模型提供事实依据
2. **知识注入**：将知识图谱的三元组转化为训练数据，注入到大模型中
3. **符号引导生成**：用知识图谱的结构约束大模型的输出
4. **混合推理**：符号推理处理确定性知识，统计推理处理不确定性
...
**关键问题**：知识图谱是"慢而准"，大模型是"快而模糊"。如何让它们**协同工作**，而不是简单拼接？

### 思考题：符号主义的现代复兴？

知识图谱与大模型的结合，是否意味着符号主义的某种**复兴**？还是说，这只是在统计框架下**借用**了符号的形式？

一个更深刻的问题：如果大模型能从文本中"学习"到知识，为什么还需要显式的知识图谱？可能的答案：**可解释性、可验证性、可更新性**——这些在关键应用（医疗、法律、金融）中不可或缺。
...
- **符号与统计的结合**：知识图谱提供结构化知识，大模型提供泛化能力。这种结合是"1+1>2"的协同，还是"油水分离"的拼接？真正的融合需要解决哪些根本问题？

- **知识的双重表示**：同一个事实，在知识图谱中是离散的三元组，在大模型中是连续的向量分布。这两种表示如何**相互验证、相互补充**？当它们冲突时，应该相信谁？
