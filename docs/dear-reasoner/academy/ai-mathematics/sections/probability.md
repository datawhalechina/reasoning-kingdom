...
- $\mathcal{F}$：事件域，$\Omega$ 的子集构成的 $\sigma$-代数
- $P$：概率测度，满足：
  1. 非负性：$P(A) \ge 0$
  2. 规范性：$P(\Omega) = 1$
  3. 可列可加性：对互斥事件 $A_1, A_2, \ldots$，$P(\bigcup_i A_i) = \sum_i P(A_i)$
...
**小小猪**举了个例子：抛一枚均匀硬币：
- $\Omega = \{正面, 反面\}$
- $\mathcal{F} = \{\emptyset, \{正面\}, \{反面\}, \Omega\}$
- $P(正面) = 1/2$，$P(反面) = 1/2$
...
问题：证明以下性质：
1. 如果 $A, B$ 独立，则 $A, \neg B$ 也独立
2. 如果 $A, B$ 独立且 $A, C$ 独立，$A$ 与 $B\cap C$ 不一定独立
3. 条件概率满足概率公理
...
- 单调性：如果 $X \le Y$，则 $E[X] \le E[Y]$

**方差性质**：
- $\text{Var}[aX+b] = a^2\text{Var}[X]$
- $\text{Var}[X+Y] = \text{Var}[X] + \text{Var}[Y] + 2\text{Cov}(X,Y)$
- 如果 $X, Y$ 独立，$\text{Var}[X+Y] = \text{Var}[X] + \text{Var}[Y]$

### 思考题1：动手题
问题：计算以下分布的期望和方差：
1. 伯努利分布 $\text{Bernoulli}(p)$
2. 均匀分布 $\text{Uniform}(a,b)$
...
$$\frac{X_1+\cdots+X_n - n\mu}{\sigma\sqrt{n}} \to N(0,1) \quad \text{（依分布）}$$
