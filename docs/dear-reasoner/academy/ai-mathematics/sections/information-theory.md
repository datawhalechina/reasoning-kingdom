...
- 偏态分布（晴90%，雨10%）：熵 $\approx 0.47$ 比特，有一定不确定性
...
**熵的极值**：
- 最小熵 $0$：确定分布（某个结果概率为1）
- 最大熵 $\log|\mathcal{X}|$：均匀分布（所有结果等可能）
- 最大熵原理：在给定约束下，选择熵最大的分布

### 思考题1：动手题
问题：计算以下分布的熵：
1. 伯努利分布 $\text{Bernoulli}(p)$：$p = 0.5, 0.1, 0.9$
2. 均匀分布：掷骰子（6面均匀）
...
1. $H(X,Y) \le H(X) + H(Y)$
2. $H(X|Y) \le H(X)$
3. $I(X;Y) = \sum_{x,y} p(x,y) \log\frac{p(x,y)}{p(x)p(y)}$
...
**兔狲教授**举例说：真实分布 $P$ vs 模型分布 $Q$：
- $P$：真实数据分布
- $Q$：模型预测分布
- $D_{KL}(P\|Q)$：用 $Q$ 近似 $P$ 的代价
- 最小化KL散度：让模型分布接近真实分布
...
**KL散度的不对称性**：
- **正向KL**：$D_{KL}(P\|Q)$，强调覆盖 $P$ 的所有模式
- **反向KL**：$D_{KL}(Q\|P)$，避免 $Q$ 在 $P$ 为零处有概率
- 在变分推断中：用反向KL得到更紧凑的近似

### 思考题1：动手题
问题：计算KL散度：
1. $P=\text{Bernoulli}(0.5)$，$Q=\text{Bernoulli}(0.7)$
2. $P=\text{Bernoulli}(0.1)$，$Q=\text{Bernoulli}(0.9)$
3. $P=\text{Bernoulli}(0.5)$，$Q=\text{Bernoulli}(0.5)$
...
**小海豹**举了个例子：二进制对称信道（BSC）：
- 输入0/1，以概率 $p$ 翻转为1/0
- 容量 $C = 1 - H(p)$（其中 $H(p)$ 是二元熵）
- 当 $p=0$（无噪声）：$C=1$（每比特可靠传输1比特）
- 当 $p=0.5$（完全随机）：$C=0$（无法可靠传输）
- 当 $p=0.1$：$C \approx 0.53$
...
2. 二进制擦除信道：以概率 $\varepsilon$ 擦除（输出？），否则正确
3. Z信道：$0\to 0$ 概率1，$1\to 0$ 概率 $p$，$1\to 1$ 概率 $1-p$
