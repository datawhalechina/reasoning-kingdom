...
1. **凸函数**：收敛到全局最优
2. **$L$-光滑**：$\|\nabla f(x) - \nabla f(y)\| \le L\|x-y\|$
3. **$\mu$-强凸**：$f(y) \ge f(x) + \nabla f(x)^T(y-x) + \mu\|y-x\|^2/2$

**收敛速度**：
- 强凸光滑：线性收敛 $O\!\left((1-\mu/L)^k\right)$
- 一般凸：次线性收敛 $O(1/k)$
- 非凸：收敛到驻点
...
$$\min_x \max_{\lambda,\nu} L_\rho(x,\lambda,\nu) = f(x) + \sum_i \lambda_i g_i(x) + \sum_j \nu_j h_j(x) + \frac{\rho}{2}\sum_i [g_i(x)]_+^2 + \frac{\rho}{2}\sum_j [h_j(x)]^2$$
