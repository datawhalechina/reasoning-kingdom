...
**小海豹**举了个例子：从空集 $\emptyset$ 开始：
1. 配对公理：$\{\emptyset, \emptyset\} = \{\emptyset\}$（单元素集）
2. 配对公理：$\{\emptyset, \{\emptyset\}\}$（两个元素的集合）
3. 并集公理：$\cup\{\{\emptyset\}, \{\{\emptyset\}\}\} = \{\emptyset, \{\emptyset\}\}$
4. 幂集公理：$P(\{\emptyset\}) = \{\emptyset, \{\emptyset\}\}$
...
**小海豹**的旅程：从空集开始：
1. 自然数：$0=\emptyset$，$1=\{\emptyset\}$，$2=\{\emptyset,\{\emptyset\}\}$，$\ldots$
2. 整数：$(a,b)$ 表示 $a-b$，如 $(3,2)=1$，$(2,3)=-1$
3. 有理数：$(a,b)$ 表示 $a/b$，$b \ne 0$
4. 实数：戴德金分割 $\{A,B\}$，A中每个数小于B中每个数
5. 复数：实数有序对 $(a,b)$ 表示 $a+bi$
