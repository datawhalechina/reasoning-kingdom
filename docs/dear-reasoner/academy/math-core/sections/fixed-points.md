...
**小海豹**举了个例子：考虑函数 $f(x)=x/2+1$ 在 $\mathbb{R}$ 上：
...
**收敛速度**：
1. **线性收敛**：$|x_{n+1}-x^*| \le k|x_n-x^*|$，$0 < k < 1$
2. **超线性收敛**：如牛顿法的二次收敛
3. **次线性收敛**：如 $|f'(x^*)|=1$ 的情况
...
3. 证明：如果f单调，则 $\mathrm{lfp}(f) = \bigvee_{n \ge 0} f^n(\bot)$，其中 $\bot$ 是最小元
